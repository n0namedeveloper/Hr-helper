...
from hr_breaker.async_runtime import run_sync
...
# ---------- ASYNC ОБЁРТКИ ЧЕРЕЗ ГЛОБАЛЬНЫЙ LOOP ----------

def extract_name_sync(content: str) -> tuple[str | None, str | None]:
    return run_sync(extract_name(content))


def parse_job_posting_sync(text: str):
    return run_sync(parse_job_posting(text))


def optimize_for_job_sync(*args, **kwargs):
    return run_sync(optimize_for_job(*args, **kwargs))
...
def cached_extract_name(content: str) -> tuple[str | None, str | None]:
    return extract_name_sync(content)


@st.cache_resource(show_spinner=False)
def cached_parse_job(text: str):
    return parse_job_posting_sync(text)
...
with st.sidebar:
    ...
    if not settings.perplexity_api_key:
        st.warning("Set PERPLEXITY_API_KEY in .env")
    ...
    try:
        ...
        with st.status("Optimizing resume...", expanded=True) as status_container:
            def on_iteration(i, opt, val):
                # только копим результаты, без st.*
                iteration_results.append((i, opt, val))

            optimized, validation, job = optimize_for_job_sync(
                source,
                job_text,
            ...
            # обновляем статус уже в основном потоке
            for i, opt, val in iteration_results:
        ...
        if optimized and optimized.pdf_bytes:
            ...
            # Создаём имя файла
            first_name = source.first_name or "Resume"
            company = job.company.replace(" ", "_")[:20]
            title = job.title.replace(" ", "_")[:20]
            pdf_filename = f"{first_name}_{company}_{title}.pdf"
            pdf_path = output_dir / pdf_filename
            
            # Сохраняем PDF
            pdf_path.write_bytes(optimized.pdf_bytes)
            logger.info(f"PDF saved to {pdf_path}")
            status_container.write(f"✓ PDF saved to {pdf_path}")
            ...
            status_container.write("⚠ No PDF bytes to save")
            logger.warning(f"Optimization produced no PDF bytes. Optimized: {optimized}, PDF bytes: {optimized.pdf_bytes if optimized else 'N/A'}")
...
if "last_result" in st.session_state:
    ...
    if validation.passed:
        ...
        st.warning(
            f"Max iterations ({len(passed)}/{len(validation.results)} passed). "
