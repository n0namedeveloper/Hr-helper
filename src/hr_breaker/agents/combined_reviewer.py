...
class CombinedReviewResult(BaseModel):
    ...
    looks_professional: bool = Field(
        default=False,
        description="True if resume looks professional"
    )
    ...
    keyword_score: float = Field(
        ...
        ge=0.0,
        le=1.0,
        description="Keyword/skills match rate"
    )
    experience_score: float = Field(
        default=0.5,
        ge=0.0,
        le=1.0,
        description="Experience alignment"
    )
    education_score: float = Field(
        default=0.5,
        ge=0.0,
        le=1.0,
        description="Education fit"
    )
    overall_fit_score: float = Field(
        default=0.5,
        ge=0.0,
        le=1.0,
        description="Holistic fit for the role"
    )
    disqualified: bool = Field(
        default=False,
        description="Auto-reject triggered"
    )
    ats_issues: list[str] = Field(
        default_factory=list,
        description="ATS-related issues"
    )
...
def get_combined_reviewer_agent() -> Agent:
    ...
    agent = create_perplexity_agent(
        ...
        result_type=CombinedReviewResult,
