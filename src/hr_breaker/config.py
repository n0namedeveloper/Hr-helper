...
class Settings(BaseModel):
    ...
    perplexity_pro_model: str = "sonar-pro"
    perplexity_flash_model: str = "sonar-pro"  # Using sonar-pro for tool support
    ...
    # API cost limits (to prevent excessive spending)
    perplexity_max_completion_tokens: int = 2000  # ~$0.02-0.024 for sonar-pro output
    perplexity_cost_limit_cents: float = 10.0  # Maximum 10 cents per API call
...
def get_settings() -> Settings:
    return Settings(
        ...
        perplexity_pro_model=os.getenv("PERPLEXITY_PRO_MODEL") or "sonar-pro",
        perplexity_flash_model=os.getenv("PERPLEXITY_FLASH_MODEL") or "sonar-pro",  # sonar-pro for tool support
        ...
        # API cost limits
        perplexity_max_completion_tokens=int(
            os.getenv("PERPLEXITY_MAX_COMPLETION_TOKENS", "2000")
        ),
        perplexity_cost_limit_cents=float(
            os.getenv("PERPLEXITY_COST_LIMIT_CENTS", "10.0")
        ),
    )


def get_model_settings() -> dict[str, Any] | None:
    """Get model settings for Perplexity."""
    # Perplexity doesn't require additional settings at the moment
    return None
